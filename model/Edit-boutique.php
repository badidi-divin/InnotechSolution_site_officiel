<?php
  ...
  if (isset($_POST['save'])) {
  	...
  	if ($name==''){
  		...
  		$ps=$pdo->prepare("UPDATE boutique SET nom_boutique=?,description=?,categorie=?,monaie=?  WHERE id_boutique=?");
		    //Pour bien recupere les données on crée un table de parametre
		    $params=array($nom,$description,$categorie,$monaie,$id);
	...
	if (in_array($extension, $extensionAutorise)) {
		...
		if ($size<=$tailleMax) {
			...
			$ps=$pdo->prepare("UPDATE boutique SET nom_boutique=?,description=?,image_boutique=?,categorie=?,monaie=? WHERE id_boutique=?");
		    //Pour bien recupere les données on crée un table de parametre
		    $params=array($nom,$description,$fileName,$categorie,$monaie,$id);
		    //Execution de la requête par leur paramètre en ordre 
		    $ps->execute($params);
