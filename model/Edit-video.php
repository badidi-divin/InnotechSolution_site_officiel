<?php
	$id=$_GET['id'];
	$requser=$pdo->prepare("SELECT * FROM video WHERE id=?");
	...
	if (isset($_POST['save'])) {
		...
		if ($video_name=='') {
				...
				$ps=$pdo->prepare("UPDATE video SET nom=?,description=?,categorie=? WHERE id=?");
			...
			if (in_array($video_ex_lc, $allowed_exs)) {
				...
				$ps=$pdo->prepare("UPDATE video SET nom=?,description=?,chemin=?,categorie=? WHERE id=?");
				//Pour bien recupere les données on crée un table de parametre
